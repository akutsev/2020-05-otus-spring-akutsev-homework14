...
@DataJpaTest
public class CommentSqlDaoTest {
	...
	public void addCommentGetByIdTest() {
		...
		long id = commentSqlDao.save(comment).getId();

		entityManager.clear();
		Comment commentFromDb = commentSqlDao.findById(id).get();
	...
	public void deleteGenreTest() {
		...
		assertNotEquals(Optional.empty(), commentSqlDao.findById(id));
		commentSqlDao.delete(commentToDelete);
		assertEquals(Optional.empty(), commentSqlDao.findById(id));
	}
	...
	public void getAllCommentsByBook() {
		...
		List<Comment> comments = commentSqlDao.findByBook(book);
