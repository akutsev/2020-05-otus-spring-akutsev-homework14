...
public class GenreSqlDaoTest {
	...
	public void addGenreGetByIdTest() {
		...
		long id = genreSqlDao.save(genre).getId();

		entityManager.clear();
		Genre genreFromDb = genreSqlDao.findById(id).get();
	...
	public void deleteGenreTest() {
		...
		assertNotEquals(Optional.empty(), genreSqlDao.findById(id));

		genreSqlDao.delete(genreToDelete);
		assertEquals(Optional.empty(), genreSqlDao.findById(id));
	}
}
